...
export function TextNode({ id, data, selected }: NodeProps) {
  ...
  const [attachedFiles, setAttachedFiles] = useState<
    { name: string; id?: string; uploading?: boolean }[]
  >([]);
  ...
  const updateMessageInNode = useCanvasStore(
    (state) => state.updateMessageInNode
  );
  const splitNodeWithContent = useCanvasStore(
    (state) => state.splitNodeWithContent
  );
  ...
  const splitMenuRef = useClickOutside<HTMLDivElement>(
      ...
      try {
        ...
        if (data.xaiFileId) {
          setAttachedFiles((prev) =>
            prev.map((f) =>
              f.name === file.name
                ? { name: f.name, id: data.xaiFileId, uploading: false }
                : f
            )
          );
        } else {
          ...
          setAttachedFiles((prev) =>
            prev.map((f) =>
              f.name === file.name ? { name: f.name, uploading: false } : f
            )
          );
        }
      } catch (err) {
  ...
  const handleSubmit = async (overridePrompt?: string) => {
    ...
    const sourceImages = connectedNodes
      .filter(
        (n) =>
          n?.type === "image" && !n.data?.images?.[0]?.url && n.data?.editImage
      )
    ...
    const manualFileIds = attachedFiles
      .map((f) => f.id)
      .filter(Boolean) as string[];
    ...
    const attachedNames = [
      ...
      ...connectedNodes.map(
        (n) => n?.data.label || n?.type || "Connected Node"
      ),
    ].join(", ");
    ...
    try {
      ...
      const currentMessages =
        (currentNode?.data.messages as ChatMessage[] | undefined) || [];
      ...
      const nodeMessages = currentMessages.map((m) => ({
        role: m.role,
        content: m.content,
      }));
  ...
  return (
    ...
    >
      {/* Header */}
      <div
        className="flex items-center justify-between p-3 border-b border-gray-800 bg-[#141414] rounded-t-xl cursor-grab active:cursor-grabbing drag-handle"
      ...
      >
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className={`p-1.5 rounded-md transition-all duration-200 nodrag ${
              webSearch
          ...
          <div className="divide-y divide-gray-800/50">
            {messages.map((msg, index) => (
              <div
                key={index}
                className={`p-3 ${
                  msg.role === "user" ? "bg-[#141414]" : "bg-transparent"
                } group/msg animate-slide-up`}
              >
      ...
      {/* Input Area */}
      <div
        className={`p-3 border-t border-gray-700 bg-[#1a1a1a] nopan ${
          !isReasoningSectionVisible ? "rounded-b-xl" : ""
        }`}
      >
        {attachedFiles.length > 0 && (
