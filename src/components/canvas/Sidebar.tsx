...
import { 
  ...
  MoreHorizontal, Clock, Trash2, LogOut, Edit2, Check, X
} from 'lucide-react';
...
export function Sidebar({ onCanvasSelect }: SidebarProps) {
  ...
  const saveRename = async (id: string, e: React.SyntheticEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setHistory(prev => prev.map(c => c.id === id ? { ...c, name: editName } : c));
        setEditingId(null);
      }
    } catch (err) {
      ...
      <div className={`flex flex-col h-full overflow-hidden ${!isOpen && 'hidden'}`}>
        ...
        <div className="px-3 pb-2">
          <div className="relative group">
            <Search size={12} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 z-10 group-focus-within:text-white transition-colors" />
            <Input 
              type="text" 
              placeholder="Search..." 
        ...
        <div className="flex-1 overflow-y-auto px-2 py-2 space-y-1">
          <div className="px-2 py-1 text-[9px] font-mono text-gray-600 uppercase tracking-widest">Recent</div>
          
          {loading ? (
            <div className="px-2 py-4 text-[10px] font-mono text-gray-600 animate-pulse">LOADING...</div>
          ) : history.length === 0 ? (
            <div className="px-2 py-4 text-[10px] font-mono text-gray-600">NO_HISTORY</div>
          ) : (
            history.map((item) => {
              ...
              return (
                ...
                >
                  <Clock size={12} className={`${isActive ? 'text-white' : 'text-gray-600 group-hover:text-gray-400'} flex-shrink-0 transition-colors`} />
                  ...
                  <div className="flex-1 min-w-0">
                    {editingId === item.id ? (
                      <div className="flex items-center gap-1" onClick={e => e.stopPropagation()}>
                        ...
                        />
                        <button onClick={(e) => saveRename(item.id, e)} className="text-green-500 hover:text-green-400 p-0.5">
                          <Check size={12} />
                        </button>
                        <button onClick={(e) => cancelEditing(e)} className="text-red-500 hover:text-red-400 p-0.5">
                          <X size={12} />
                        </button>
                      </div>
                    ) : (
                      <>
                        <div className={`text-xs truncate transition-colors font-medium ${isActive ? 'text-white' : 'text-gray-400 group-hover:text-gray-200'}`}>
                          {item.name}
                        </div>
                        <div className={`text-[9px] font-mono transition-colors ${isActive ? 'text-gray-500' : 'text-gray-700'}`}>
                  ...
                  {editingId !== item.id && (
                    <div className="relative">
                      <button 
                        className={`opacity-0 group-hover:opacity-100 p-1 rounded transition-all ${isActive ? 'text-gray-400 hover:text-white' : 'text-gray-600 hover:text-gray-300'}`}
                        title="More options"
                        onClick={(e) => {
                          e.stopPropagation();
        ...
        <div className="p-3 border-t border-white/5 bg-[#050505]">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 text-gray-500 hover:text-red-400 hover:bg-white/5 py-2 rounded-lg text-xs font-mono uppercase tracking-wider transition-colors"
          >
            <LogOut size={14} />
            Sign Out
          </button>
